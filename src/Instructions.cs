...
internal class InstructionInfo
{
    internal class Macros 
    {
        ...
        public void SwitchMacro(Expr.Function f)
        {
            if (f.constructor)
            {
                header = MacroType.EmptyHeader;
                footer = MacroType.EmptyFooter;
            }
            else if (f.keepStack)
            {
                size = f.size;
                header = MacroType.FunctionHeader2;
                footer = MacroType.FunctionFooter2;
            }
            else
            {
                header = MacroType.FunctionHeader;
        ...
        public void SwitchMacro(MacroType headerMacro, MacroType footerMacro)
        {
            header = headerMacro;
            footer = footerMacro;
        }
        public void DoMacroHeader(List<Instruction> instructions)
        {
            DoMacro(instructions, header);
        }
        public void DoMacroFooter(List<Instruction> instructions)
        {
            DoMacro(instructions, footer);
        }

        public void DoMacro(List<Instruction> instructions, MacroType? macroType)
        {
            switch (macroType)
            {
                case MacroType.EmptyHeader:

                    break;
                case MacroType.FunctionHeader:

                    instructions.Add(new Instruction.Unary("PUSH", "RBP"));
                    instructions.Add(new Instruction.Binary("MOV", "RBP", "RSP"));
                    break;
                case MacroType.FunctionHeader2:

                    instructions.Add(new Instruction.Unary("PUSH", "RBP"));
                    instructions.Add(new Instruction.Binary("MOV", "RBP", "RSP"));
                    if (size == null) { throw new NullReferenceException(); } 
                    instructions.Add(new Instruction.Binary("SUB", "RSP", size.ToString()));
                    break;
                case MacroType.EmptyFooter:

                    break;
                case MacroType.FunctionFooter:

                    instructions.Add(new Instruction.Unary("POP", "RBP"));
                    instructions.Add(new Instruction.Zero("RET"));
                    break;
                case MacroType.FunctionFooter2:

                    instructions.Add(new Instruction.Zero("LEAVE"));
                    instructions.Add(new Instruction.Zero("RET"));
                    break;
                default:
                    break;
