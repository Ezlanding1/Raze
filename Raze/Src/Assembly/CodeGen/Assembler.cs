...
public class Assembler : Expr.IVisitor<Instruction.Value?>
{
    ...
    Instruction.Value? Expr.IVisitor<Instruction.Value?>.VisitWhileExpr(Expr.While expr)
    {
        Emit(new Instruction.Unary("JMP", new Instruction.LocalProcedureRef(ConditionalLabel)));
    ...
    internal Instruction.Register MovToRegister(Instruction.Value operand, Instruction.Register.RegisterSize? size) => operand.NonPointerNonLiteral(size, this);
